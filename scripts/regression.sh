...
grep -q '^capability_response_guard=installed$' "$TEMP_REPORT"
grep -q '^interrupted_thread_reset=true$' "$TEMP_REPORT"
grep -q '^native_stop_patch=installed$' "$TEMP_REPORT"
grep -q '^capability_request_gate=installed$' "$TEMP_REPORT"
grep -q '^generic_image_generation=unsupported$' "$TEMP_REPORT"
...
grep -q '^markdown_delivery_patch=installed$' "$TEMP_REPORT"
