...
- 每轮收口：形成最终答复后静默提取候选，没有候选时不写入。
- 普通会话：仅在产生稳定事实、偏好、项目决策、明确待办或可复用标准时写入收件箱。
- 明确“记住”：直接成为当前有效长期记忆。
- 模型归纳：仍需核对的内容只进入待确认层，不直接生效。
- 内容冲突：必须建立“旧版本 → 新版本”的取代关系。
- 停止使用：创建逻辑停用记录；不物理删除历史账本。
- 密码、Token、API Key、SSH Key、身份证号和银行卡号拒绝写入。

同一轮多个候选通过 `close-turn` 一次提交。工具先完成整批格式、敏感信息和冲突
检查，再追加账本，因此无效候选不会造成半批写入。相同记忆会去重；用户后来明确
要求记住已有候选时，原记录直接晋升，不生成副本。
